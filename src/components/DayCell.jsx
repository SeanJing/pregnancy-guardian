    ...
    >
      ...
      {hasContent && (
        <div className="mt-1 flex gap-0.5 flex-wrap">
          {contentFlags.todos && <span className="w-1.5 h-1.5 rounded-full bg-primary" />}
          {contentFlags.pics && <span className="w-1.5 h-1.5 rounded-full bg-cta" />}
          {contentFlags.note && <span className="w-1.5 h-1.5 rounded-full bg-ink/30" />}
        </div>
      )}
    </button>
  )
