...
export default function EventsSection({ events: initialEvents, date, updateDay }) {
  ...
  const add = async (e) => {
    ...
    sync([...events, { id: res.id, text: text.trim(), time, done: false }])
    setText(''); setTime('')
  }
      ...
      <ul className="space-y-1.5">
        {events.map(t => (
          <li key={t.id} className="flex items-center gap-2 group">
            ...
            <span className={`flex-1 text-sm ${t.done ? 'line-through text-ink/40' : ''}`}>{t.time && <span className="text-ink/40 mr-1.5">{t.time}</span>}{t.text}</span>
