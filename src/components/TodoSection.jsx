...
export default function TodoSection({ todos, onUpdate }) {
  const [text, setText] = useState('')

  const add = (e) => {
    e.preventDefault()
    if (!text.trim()) return
    onUpdate([...todos, { text: text.trim(), done: false }])
    setText('')
  }

  const toggle = (i) => onUpdate(todos.map((t, j) => j === i ? { ...t, done: !t.done } : t))
  const remove = (i) => onUpdate(todos.filter((_, j) => j !== i))
      ...
      <ul className="space-y-1.5">
        {todos.map((t, i) => (
          <li key={i} className="flex items-center gap-2 group">
            <button onClick={() => toggle(i)} className={`shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center cursor-pointer transition-colors duration-150 ${t.done ? 'bg-primary border-primary' : 'border-gray-300'}`} aria-label="Toggle task">
            ...
            <button onClick={() => remove(i)} className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-red-50 cursor-pointer transition-opacity duration-150" aria-label="Delete task">
