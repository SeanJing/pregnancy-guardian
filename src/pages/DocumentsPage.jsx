...
export default function DocumentsPage() {
  ...
  const renderRow = (doc) => (
    <li key={doc.id} className="flex items-center gap-4 py-3 group">
      {getIcon(doc.name)}
      <a href={UPLOADS_BASE + doc.url} target="_blank" rel="noopener noreferrer" className="flex-1 min-w-0 cursor-pointer hover:text-primary transition-colors duration-150">
  ...
  return (
    <>
      ...
      <div className="px-4 md:px-6 lg:px-8 pb-8">
        {loading ? <DocumentsSkeleton /> : error ? <ErrorMessage message={error} onRetry={load} /> : docs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-ink/40">
            ...
            <p className="text-sm">No documents yet. Tap "Add Document" to add files.</p>
          </div>
        ) : searchedDocs.length === 0 ? (
          <p className="text-center text-ink/40 text-sm py-16">No results for "{search}"</p>
        ) : viewBy === 'all' ? (
          <ul className="divide-y divide-gray-100">{searchedDocs.map(renderRow)}</ul>
        ) : viewBy === 'date' ? (
