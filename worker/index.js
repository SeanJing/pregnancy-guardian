...
async function initDB(db) {
  await db.exec("CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT NOT NULL, text TEXT NOT NULL, time TEXT DEFAULT '');")
...
app.get('/api/calendar', async (c) => {
  ...
  for (const r of (await db.prepare(`SELECT id, date, time, text FROM events${where} ORDER BY id`).bind(...params).all()).results) {
    ensure(r.date); data[r.date].events.push({ id: r.id, text: r.text, time: r.time || '' })
  }
...
app.post('/api/events', async (c) => {
  const db = c.env.DB
  const { date, text, time } = await c.req.json()
  const res = await db.prepare('INSERT INTO events (date, text, time) VALUES (?, ?, ?) RETURNING id').bind(date, text, time || '').first()
  return c.json({ id: res.id, date, text, time: time || '' })
})

app.put('/api/events/:id', async (c) => {
  const db = c.env.DB
  const { text, time } = await c.req.json()
  await db.prepare('UPDATE events SET text=?, time=? WHERE id=?').bind(text || '', time || '', Number(c.req.param('id'))).run()
